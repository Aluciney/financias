...
- **Logging:** `src/libs/logger.ts` exporta um logger Pino único (nível via `LOG_LEVEL`, pretty em `development`, JSON em prod/test). O Fastify usa essa mesma instância (`loggerInstance`), então logs de request e de negócio compartilham formato. Em services/libs use `criarLogger({ modulo: '...' })` (child logger com contexto) — ex.: `fiis.service.ts` loga cadastro/atualização, `statusInvest.ts` loga o scraping.
