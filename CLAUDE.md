...
- **`src/libs/statusInvest.ts`** — scraping (axios + cheerio + iconv-lite/latin1). Calcula `dyMensal` e `dyAnual` a partir de `provento / cotação`. É a fonte de toda a coluna de "retorno"; ordenação por melhor opção = `dy_anual desc` no DAO.
...
- O scraping depende do StatusInvest estar acessível; tickers inexistentes (ex.: SNAG11) retornam página de "não encontrado" e o cadastro falha com 404.
